...
public class OBJLoader
{
    ...
    public static Model load(Path p)
    {
    		...
    		while(in.hasNextLine())
    		{
    			...
    			else if(line.startsWith(FACE))
    			{
    				...
    				for(int i = 1; i < splits.length; i++)
    				{
    				    ...
    				    localVertices.add(vertices.get(Integer.parseInt(indices[0]) - 1));
    				    //TODO add uv coords
    	...
    	Logging.fine("Loaded " + p.toString() + " - " + model.faceCount() + " faces, " + model.verticesCount() + " vertices, " + vertices.size() + " unique vertices.");
    	return model;
    }
}